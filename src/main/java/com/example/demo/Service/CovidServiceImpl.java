...
public class CovidServiceImpl implements CovidService{
    ...
    @Override
    public List<CovidServiceResponse> getCovidData() {


        HttpHeaders httpHeaders = setHeaders();

        HttpEntity<String> entity = new HttpEntity<>(httpHeaders);
        ResponseEntity<List<CovidServiceResponse>> response = null;
        try {
            ...
            response = restTemplate.exchange("https://covid-19-data.p.rapidapi.com/report/country/name?date-format=YYYY-MM-DD&format=json&date=2020-04-01&name=USA",
                    HttpMethod.GET, entity, new ParameterizedTypeReference<List<CovidServiceResponse>>() {});
        }catch(Exception e){
            e.printStackTrace();
            return null;
        }
